				...
				<c:forEach items="${items}" var="item">
					...
					<fieldset id="item${counter}" class="border border-success p-3">
						...
						<button type="button" class="qdown"
							onClick="quantityDown(${counter}, ${item.getID()}, ${item.getPrice()}, './Cart?out=changeItem')">-
						</button>
						<c:set var="split2"
						...
						<button type="button" class="qup"
							onClick="quantityUp(${counter}, ${item.getID()}, ${item.getPrice()}, ${item.getQuantity()}, './Cart?out=changeItem')">+</button>
